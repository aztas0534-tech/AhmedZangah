...
async function run() {
    const { data: allRet, error: retErr } = await supabase.from('purchase_returns').select('id, order_id, status, refund_method, total_refund_amount, created_at').order('created_at', { ascending: false });
    if (retErr) {
    ...
    if (target) {
        const { data: je, error: jeErr } = await supabase.from('journal_entries').select('id, source_table, source_id, source_event, status').eq('source_table', 'purchase_returns').eq('source_id', target.id).maybeSingle();
