...
let results = [];
let lines = funcsStr.split('\n');
for (let i = 0; i < lines.length; i++) {
  const line = lines[i].toLowerCase();
  
  // if line has "update" and "data", let's check further
  if (line.includes('update ') && (line.includes(' data ') || line.includes(' data='))) {
...
if (results.length > 0) {
    console.log("Found suspicious static updates on missing data columns!!");
    console.log(JSON.stringify(results, null, 2));
} else {
    console.log("No static update data column conflicts found across " + missingTables.length + " tables in all functions.");
}
