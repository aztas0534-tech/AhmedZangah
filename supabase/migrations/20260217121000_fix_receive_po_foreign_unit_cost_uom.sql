...
begin
  ...
  begin
    with calc as (
      select
        ...
        greatest(
          ...
          - (coalesce(pri.transport_cost, 0) / nullif(pri.quantity, 0))
          - (coalesce(pri.supply_tax_cost, 0) / nullif(pri.quantity, 0))
        ) as base_goods_unit_cost
      from public.batches b
      join public.purchase_receipt_items pri
        on pri.receipt_id = b.receipt_id
       and pri.item_id = b.item_id
    ...
    set
      foreign_unit_cost = round(
        c.base_goods_unit_cost / nullif(b.fx_rate_at_receipt, 0),
        6
      ),
      updated_at = now()
    from calc c
    where b.id = c.id
      and c.base_goods_unit_cost is not null
      and (
        b.foreign_unit_cost is null
        or abs((b.foreign_unit_cost * b.fx_rate_at_receipt) - c.base_goods_unit_cost)
            > greatest(0.01, c.base_goods_unit_cost * 0.05)
      );
  exception when others then
    if v_has_trigger then
...
begin
  if to_regclass('public.inventory_movements') is not null and to_regclass('public.batches') is not null then
    ...
    set unit_cost = round(coalesce(b.unit_cost, 0), 6),
    ...
    where b.id = im.batch_id
      ...
      and abs(
        ...
        - (coalesce(im.quantity, 0) * round(coalesce(b.unit_cost, 0), 6))
...
begin
  ...
  with candidates as (
    select
      je.id as entry_id,
      upper(coalesce(nullif(btrim(je.currency_code), ''), v_base)) as current_currency_code,
      je.fx_rate as current_fx_rate,
      je.foreign_amount as current_foreign_amount,
  ...
  line_stats as (
    ...
    join candidates c
      on c.entry_id = jl.journal_entry_id
  ...
  fixable as (
    ...
    from candidates c
    join line_stats ls
      on ls.entry_id = c.entry_id
    where ls.line_count = 2
      and ls.debit_lines = 1
      and ls.credit_lines = 1
      and abs(coalesce(ls.total_debit, 0) - coalesce(c.expected_base, 0))
          > greatest(0.01, abs(coalesce(ls.total_debit, 0)) * 0.05)
  )
  ...
  fixable as (
    ...
    where ls.line_count = 2
      ...
      and ls.credit_lines = 1
      and (
        c.current_currency_code is distinct from c.currency_code
        or c.current_fx_rate is distinct from c.fx_rate
        or abs(coalesce(c.current_foreign_amount, 0) - coalesce(c.expected_foreign, 0))
            > greatest(0.01, abs(coalesce(c.current_foreign_amount, 0)) * 0.05)
      )
  )
