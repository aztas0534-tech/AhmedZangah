do $$
begin
  if to_regclass('public.pricing_rpc_logs') is not null then
    grant select on table public.pricing_rpc_logs to authenticated;
  end if;
end $$;

do $$
begin
  if to_regprocedure('public.get_pitr_status()') is not null then
    revoke execute on function public.get_pitr_status() from authenticated;
...
begin
  ...
  if v_batch.id is null then
    ...
    v_auth := coalesce(auth.role(), current_setting('request.jwt.claim.role', true), 'unknown');
    v_user := auth.uid();
  ...
  if reason_code is not null or warning_next_batch_price_diff or random() < 0.01 then
    v_auth := coalesce(auth.role(), current_setting('request.jwt.claim.role', true), 'unknown');
    v_user := auth.uid();
    insert into public.pricing_rpc_logs(auth_role, auth_user_id, function_name, item_id, warehouse_id, quantity, currency_code, customer_id, suggested_price, min_price, batch_id, reason_code, context)
