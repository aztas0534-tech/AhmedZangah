...
-- Fix confirm_order_delivery to enforce party credit limits using the correct transaction currency.
-- Previously, it used the base amount (v_net_ar) without specifying the currency code, 
-- causing it to fall back to the base currency limit which might be 0.
...
begin
  ...
  v_currency_code := coalesce(v_order.currency, public.get_base_currency());
  ...
  -- Calculate deposits in both base and foreign currencies
  ...
  if v_is_credit and v_party_id is not null and v_net_ar > 0 then
    -- Modified check to use transaction currency
    v_ok := public.check_party_credit_limit(v_party_id, v_net_ar_foreign, v_currency_code);
    
    if not v_ok then
      -- Fetch current limits for logging the override based on the transaction currency
      ...
      -- Fallback to base currency limits if no limit defined for the specific currency
