-- Fix process_sales_return to assign 'pending' instead of 'approved' to returned batches
-- and fix any existing stuck 'approved' batches so they show up in the QC UI.
...
-- 2. Update process_sales_return (from 20260227025200_fix_returns_party_currency_uom.sql)
-- We'll replace the word 'approved' with 'pending' in the insert
  ...
  loop
    ...
    loop
      ...
      values (
        ...
        v_alloc, 0, coalesce(v_source_batch.unit_cost, 0), 'pending',
