-- ═══════════════════════════════════════════════════════════════
-- purge_order_payment v7: Fix FK — also delete party_open_items
-- and settlement_lines/settlement_headers before journal_lines
-- Error fixed: party_open_items_journal_line_id_fkey violation
-- ═══════════════════════════════════════════════════════════════
...
begin
  ...
  -- Disable ALL triggers that might block deletion
  ...
  -- settlement immutability triggers
  begin alter table public.settlement_lines disable trigger trg_settlement_lines_immutable; exception when others then null; end;
  begin alter table public.settlement_headers disable trigger trg_settlement_headers_immutable; exception when others then null; end;
  ...
  if array_length(v_jl_ids, 1) > 0 then
    -- Delete party_open_items FIRST (this was the missing step!)
    begin
  ...
  begin alter table public.settlement_lines enable trigger trg_settlement_lines_immutable; exception when others then null; end;
  begin alter table public.settlement_headers enable trigger trg_settlement_headers_immutable; exception when others then null; end;
