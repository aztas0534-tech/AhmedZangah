...
begin
  ...
  if exists (
    ...
    where pi.item_id::text = v_item_id
      ...
      and abs(coalesce(pi.qty_base, 0) - public.item_qty_to_base(pi.item_id, coalesce(pi.quantity, 0), pi.uom_id))
          > greatest(0.01, abs(public.item_qty_to_base(pi.item_id, coalesce(pi.quantity, 0), pi.uom_id)) * 0.01)
  ) then
    raise exception 'فشل إصلاح تكلفة الصنف: تحويلات الوحدة للصنف غير منضبطة. صحّح UOM ثم أعد المحاولة، أو استخدم "تعديل تكلفة الدفعة".';
  end if;
      ...
      select jsonb_build_object(
        ...
        'receiptItemsNeedingFix',
          coalesce((
            ...
            where coalesce(f.expected_unit_cost_base, 0) > 0
              and coalesce(pri2.unit_cost, 0) > 0
              and f.expected_unit_cost_base + 1e-9 < pri2.unit_cost
              and abs(coalesce(pri2.unit_cost, 0) - coalesce(f.expected_unit_cost_base, 0))
        ...
        'batchesNeedingFix',
          coalesce((
            ...
            where b2.item_id::text = v_item_id
              ...
              and coalesce(pri2.unit_cost, 0) > 0
              and coalesce(b2.unit_cost, 0) > 0
              and pri2.unit_cost + 1e-9 < b2.unit_cost
  ...
  begin
    ...
    pri_candidates as (
      ...
      where pri.item_id::text = v_item_id
        ...
        and (avg.goods_unit_cost_base + coalesce(pri.transport_cost, 0) + coalesce(pri.supply_tax_cost, 0)) + 1e-9 < coalesce(pri.unit_cost, 0)
    ...
    where b.item_id::text = v_item_id
      ...
      and pri.unit_cost + 1e-9 < b.unit_cost
