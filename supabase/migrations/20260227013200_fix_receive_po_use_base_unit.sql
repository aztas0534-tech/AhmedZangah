...
-- ============================================================================
-- CRITICAL FIX: _receive_purchase_order_partial_impl
-- Problem:  When pi.qty_base is null/0, the unit_cost fallback uses
--           pi.unit_cost (per-purchase-UOM, e.g. per carton) directly.
--           But v_take_qty is in base units → batch unit_cost = carton_cost.
-- Fix:      Add item_unit_cost_to_base() fallback when pi.uom_id is set.
-- Also:     Use base_unit instead of unit_type for stock_management insert.
-- ============================================================================
...
begin
  ...
  v_idempotency_key := nullif(
    btrim(coalesce((p_items->0->>'idempotencyKey'), (p_items->0->>'idempotency_key'))),
    ''
  );
  ...
  if v_po.status = 'cancelled' then raise exception 'cannot receive cancelled purchase order'; end if;

  v_po_fx_rate := coalesce(v_po.fx_rate, 1);
  if v_po_fx_rate <= 0 then v_po_fx_rate := 1; end if;

  begin
    update public.purchase_orders set fx_locked = true where id = p_order_id and coalesce(fx_locked, false) = false;
  ...
  exception when others then v_import_shipment_id := null;
  end;
  ...
  begin
    insert into public.purchase_receipts(
      purchase_order_id, received_at, created_by, approval_status, approval_request_id,
      requires_approval, warehouse_id, branch_id, company_id, import_shipment_id,
      idempotency_key, posting_status
    ) values (
      p_order_id, coalesce(p_occurred_at, now()), auth.uid(),
      case when v_receipt_req_id is null then 'pending' else 'approved' end,
      v_receipt_req_id, v_required_receipt, v_wh, v_po.branch_id, v_po.company_id,
      v_import_shipment_id, v_idempotency_key, 'pending'
    ) returning id into v_receipt_id;
  exception when unique_violation then
    if v_idempotency_key is not null then
      select pr.id into v_existing_receipt_id from public.purchase_receipts pr
      where pr.purchase_order_id = p_order_id and pr.idempotency_key = v_idempotency_key
      order by pr.created_at desc limit 1;
      if v_existing_receipt_id is not null then
        v_receipt_id := v_existing_receipt_id; v_reuse_receipt := true;
      else raise;
      end if;
    else raise;
    end if;
  ...
  loop
    ...
    v_expiry_iso := null; v_harvest_iso := null; v_category := null;
    ...
    if v_item_id is null or v_item_id = '' then raise exception 'Invalid itemId'; end if;
    ...
    if v_uom_id is null then
      select pi.uom_id into v_uom_id from public.purchase_items pi
      where pi.purchase_order_id = p_order_id and pi.item_id = v_item_id and pi.uom_id is not null
      order by pi.created_at desc nulls last, pi.id desc limit 1;
    end if;

    v_qty_base := public.item_qty_to_base(v_item_id, v_qty, v_uom_id);

    select coalesce(sum(coalesce(pri.quantity, 0)), 0) into v_existing_qty
    from public.purchase_receipt_items pri where pri.receipt_id = v_receipt_id and pri.item_id = v_item_id;

    v_apply_qty := greatest(v_qty_base - coalesce(v_existing_qty, 0), 0);
    if coalesce(v_apply_qty, 0) <= 0 then continue; end if;

    if v_reuse_receipt then
      update public.purchase_receipts
      set posting_status = 'pending', posting_error = null, posted_at = null,
    ...
    perform 1 from public.purchase_items pi
    where pi.purchase_order_id = p_order_id and pi.item_id = v_item_id for update;
    if not found then raise exception 'item % not found in purchase order', v_item_id; end if;
    ...
    where pi.purchase_order_id = p_order_id and pi.item_id = v_item_id;
    ...
    -- ====================================================================
    -- CRITICAL FIX: unit_cost computation
    -- pi.unit_cost is per PURCHASE UOM (e.g. per carton).
    -- We need per BASE UOM (e.g. per piece).
    -- Priority: unit_cost_base > derive from qty_base > item_unit_cost_to_base > raw fallback
    -- ====================================================================
    v_cost_sum := 0;
    ...
    for v_pi in
      select
        ...
        coalesce(
          pi.qty_base,
          case when pi.uom_id is not null then public.item_qty_to_base(pi.item_id, pi.quantity, pi.uom_id)
               else pi.quantity end,
          0
        ) as quantity,
        coalesce(pi.received_quantity, 0) as received_quantity,
        (
          case
            -- 1) unit_cost_base already set → use directly (already per-base in base currency)
            when pi.unit_cost_base is not null and pi.unit_cost_base > 0 then
              pi.unit_cost_base
            -- 2) qty_base available → derive: cost_per_base = cost_per_trx × (trx_qty / base_qty)
            when coalesce(pi.qty_base, 0) > 0 and coalesce(pi.quantity, 0) > 0 then
              coalesce(pi.unit_cost, 0)
              * (coalesce(pi.quantity, 0) / pi.qty_base)
              * (case when upper(v_po.currency) <> upper(v_base_currency) then v_po_fx_rate else 1 end)
            -- 3) uom_id available → use item_unit_cost_to_base()
            when pi.uom_id is not null then
              public.item_unit_cost_to_base(pi.item_id, coalesce(pi.unit_cost, 0), pi.uom_id)
              * (case when upper(v_po.currency) <> upper(v_base_currency) then v_po_fx_rate else 1 end)
            -- 4) fallback (no UOM info → assume unit_cost is already per base)
            else
              coalesce(pi.unit_cost, 0)
              * (case when upper(v_po.currency) <> upper(v_base_currency) then v_po_fx_rate else 1 end)
          end
          ...
          case
            when upper(v_po.currency) <> upper(v_base_currency) then
              coalesce(
                ...
                case
                  when pi.unit_cost_base is not null and pi.unit_cost_base > 0 and coalesce(v_po_fx_rate, 1) > 0 then
                    pi.unit_cost_base / v_po_fx_rate
                  when coalesce(pi.qty_base, 0) > 0 and coalesce(pi.quantity, 0) > 0 then
                    coalesce(pi.unit_cost, 0) * (coalesce(pi.quantity, 0) / pi.qty_base)
                  when pi.uom_id is not null then
                    public.item_unit_cost_to_base(pi.item_id, coalesce(pi.unit_cost, 0), pi.uom_id)
    ...
    loop
      ...
      if v_take_qty <= 0 then exit; end if;

      update public.purchase_items set received_quantity = coalesce(received_quantity, 0) + v_take_qty where id = v_pi.id;
    ...
    -- FIX: use base_unit instead of unit_type
    ...
    select v_item_id, v_wh, 0, 0, 0, coalesce(mi.base_unit, mi.unit_type, 'piece'), 5, now(), '{}'::jsonb
    ...
    from public.stock_management sm where sm.item_id::text = v_item_id and sm.warehouse_id = v_wh for update;

    select
      coalesce(v_used_transport_cost, coalesce(mi.transport_cost, 0)),
      coalesce(v_used_supply_tax_cost, coalesce(mi.supply_tax_cost, 0)),
      ...
      mi.category, coalesce(mi.is_food, false), coalesce(mi.expiry_required, false)
    ...
    if v_expiry is not null and v_expiry <> '' then
      if v_expiry ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}$' then v_expiry_iso := v_expiry;
      else raise exception 'invalid expiryDate for item %', v_item_id;
      end if;
    end if;
    if v_harvest is not null and v_harvest ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}$' then v_harvest_iso := v_harvest; end if;
    ...
    insert into public.batches(
      ...
      foreign_currency, foreign_unit_cost, fx_rate_at_receipt, fx_rate_date, data
    ) values (
      gen_random_uuid(), v_item_id, null, v_receipt_id, v_wh,
      null,
      ...
      v_apply_qty, 0, 0,
      v_effective_unit_cost, v_qc_status, 'active',
      case when upper(v_po.currency) <> upper(v_base_currency) then upper(v_po.currency) else null end,
      case when upper(v_po.currency) <> upper(v_base_currency) then v_unit_cost_foreign else null end,
      case when upper(v_po.currency) <> upper(v_base_currency) then v_po_fx_rate else null end,
      case when upper(v_po.currency) <> upper(v_base_currency) then current_date else null end,
      jsonb_build_object(
        'purchaseOrderId', p_order_id, 'purchaseReceiptId', v_receipt_id,
        'expiryDate', v_expiry_iso, 'harvestDate', v_harvest_iso,
        'warehouseId', v_wh,
        'transportCost', v_used_transport_cost, 'supplyTaxCost', v_used_supply_tax_cost,
        'importShipmentId', case when v_import_shipment_id is null then null else v_import_shipment_id::text end,
        'trxQty', v_qty,
    ...
    if v_qc_status = 'pending' then
      ...
      set qc_hold_quantity = coalesce(qc_hold_quantity, 0) + v_apply_qty,
          avg_cost = v_new_avg, last_batch_id = v_batch_id, last_updated = now(), updated_at = now()
    ...
    insert into public.inventory_movements(
      ...
      jsonb_build_object(
        'purchaseOrderId', p_order_id, 'purchaseReceiptId', v_receipt_id,
        'batchId', v_batch_id, 'expiryDate', v_expiry_iso, 'harvestDate', v_harvest_iso,
        'warehouseId', v_wh,
        'importShipmentId', case when v_import_shipment_id is null then null else v_import_shipment_id::text end
      ),
      v_batch_id, v_wh
  ...
  if v_post_failed then
    ...
    else v_post_status := 'failed';
    end if;
    update public.purchase_receipts
    set posting_status = v_post_status, posting_error = left(coalesce(v_post_error, ''), 2000), posted_at = null
    where id = v_receipt_id;
  elsif v_mark_posted then
    update public.purchase_receipts
    set posting_status = 'posted', posting_error = null, posted_at = now()
    where id = v_receipt_id;
  end if;

  for v_item_id, v_ordered, v_received in
    select pi.item_id,
      coalesce(pi.qty_base, case when pi.uom_id is not null then public.item_qty_to_base(pi.item_id, pi.quantity, pi.uom_id) else pi.quantity end, 0),
      coalesce(pi.received_quantity, 0)
    from public.purchase_items pi where pi.purchase_order_id = p_order_id
  loop
    if (v_received + 1e-9) < v_ordered then v_all_received := false; exit; end if;
  end loop;

  v_required_po := false; v_po_req_id := null; v_po_approved := true;
  ...
  if v_all_received then
    ...
    update public.purchase_orders set status = 'partial', updated_at = now() where id = p_order_id;
  end if;
