-- =======================================================================
-- Dashboard KPI: Full Date Filter Coverage
--
-- Previously, PO status counts and active order status counts
-- were NOT filtered by the selected date range.
-- Now ALL data respects p_start_date / p_end_date.
-- =======================================================================
...
begin
  ...
  -- ═══════════════════════════════════════════════════════════════
  -- SALES KPIs (date-filtered via ranged_orders CTE)
  -- ═══════════════════════════════════════════════════════════════
  with effective_orders as (
    select
      ...
      (coalesce(public.safe_cast_numeric(o.data->>'taxAmount'), 0) * public.order_fx_rate(
          coalesce(nullif(btrim(coalesce(o.currency, '')), ''), nullif(btrim(coalesce(o.data->>'currency', '')), ''), public.get_base_currency()),
          case when p_invoice_only then nullif(o.data->'invoiceSnapshot'->>'issuedAt', '')::timestamptz
            else coalesce(nullif(o.data->'invoiceSnapshot'->>'issuedAt', '')::timestamptz, nullif(o.data->>'paidAt', '')::timestamptz, nullif(o.data->>'deliveredAt', '')::timestamptz, o.created_at) end,
          o.fx_rate
        )) as tax_amount,
      (coalesce(public.safe_cast_numeric(o.data->>'deliveryFee'), 0) * public.order_fx_rate(
          coalesce(nullif(btrim(coalesce(o.currency, '')), ''), nullif(btrim(coalesce(o.data->>'currency', '')), ''), public.get_base_currency()),
          case when p_invoice_only then nullif(o.data->'invoiceSnapshot'->>'issuedAt', '')::timestamptz
            else coalesce(nullif(o.data->'invoiceSnapshot'->>'issuedAt', '')::timestamptz, nullif(o.data->>'paidAt', '')::timestamptz, nullif(o.data->>'deliveredAt', '')::timestamptz, o.created_at) end,
          o.fx_rate
        )) as delivery_fee,
      (coalesce(public.safe_cast_numeric(o.data->>'discountAmount'), 0) * public.order_fx_rate(
          coalesce(nullif(btrim(coalesce(o.currency, '')), ''), nullif(btrim(coalesce(o.data->>'currency', '')), ''), public.get_base_currency()),
          case when p_invoice_only then nullif(o.data->'invoiceSnapshot'->>'issuedAt', '')::timestamptz
            else coalesce(nullif(o.data->'invoiceSnapshot'->>'issuedAt', '')::timestamptz, nullif(o.data->>'paidAt', '')::timestamptz, nullif(o.data->>'deliveredAt', '')::timestamptz, o.created_at) end,
          o.fx_rate
        )) as discount_amount,
      (coalesce(public.safe_cast_numeric(o.data->>'subtotal'), 0) * public.order_fx_rate(
          coalesce(nullif(btrim(coalesce(o.currency, '')), ''), nullif(btrim(coalesce(o.data->>'currency', '')), ''), public.get_base_currency()),
          case when p_invoice_only then nullif(o.data->'invoiceSnapshot'->>'issuedAt', '')::timestamptz
            else coalesce(nullif(o.data->'invoiceSnapshot'->>'issuedAt', '')::timestamptz, nullif(o.data->>'paidAt', '')::timestamptz, nullif(o.data->>'deliveredAt', '')::timestamptz, o.created_at) end,
          o.fx_rate
  ...
  begin
    if to_regclass('public.sales_returns') is not null then
      select coalesce(sum(sr.total_refund_amount * coalesce(o.fx_rate, 1)), 0)
      ...
      where sr.status = 'completed'
        ...
        and (p_zone_id is null or coalesce(
            o.delivery_zone_id,
            case when nullif(o.data->>'deliveryZoneId','') is not null
                 and (o.data->>'deliveryZoneId') ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
              then (o.data->>'deliveryZoneId')::uuid else null end
          ) = p_zone_id);
    end if;
  exception when others then v_total_returns := 0;
  end;

  -- ═══════════════════════════════════════════════════════════════
  -- COGS (same as before — already date-filtered)
  -- ═══════════════════════════════════════════════════════════════
  begin
    with effective_orders as (
      select o.id, o.warehouse_id, o.data,
        case when p_invoice_only then nullif(o.data->'invoiceSnapshot'->>'issuedAt', '')::timestamptz
          else coalesce(nullif(o.data->'invoiceSnapshot'->>'issuedAt', '')::timestamptz, nullif(o.data->>'paidAt', '')::timestamptz, nullif(o.data->>'deliveredAt', '')::timestamptz, o.created_at) end as date_by,
        coalesce(o.delivery_zone_id, case when nullif(o.data->>'deliveryZoneId','') is not null and (o.data->>'deliveryZoneId') ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' then (o.data->>'deliveryZoneId')::uuid else null end) as zone_effective
      from public.orders o
      where (p_warehouse_id is null or o.warehouse_id = p_warehouse_id)
        and nullif(trim(coalesce(o.data->>'voidedAt','')), '') is null
    ),
    ranged_orders as (
      select * from effective_orders eo
      where eo.date_by >= p_start_date and eo.date_by <= p_end_date
        and (p_zone_id is null or eo.zone_effective = p_zone_id)
    ),
    eligible_orders as (
      select o.id from public.orders o join ranged_orders ro on ro.id = o.id
      where o.status = 'delivered' or o.data->>'paidAt' is not null
    ),
    oic_by_order as (
      select oic.order_id, sum(coalesce(oic.total_cost, 0)) as total_cost
      from public.order_item_cogs oic join eligible_orders eo on eo.id = oic.order_id
      group by oic.order_id
    ),
    im_by_order as (
      select (im.reference_id)::uuid as order_id,
        sum(coalesce(nullif(im.total_cost, 0), coalesce(im.quantity, 0) * coalesce(nullif(b.unit_cost, 0), nullif(im.unit_cost, 0), 0))) as total_cost
    ...
    est_by_order as (
      select o.id as order_id,
        sum(coalesce(q.qty, 0) * coalesce(nullif(sm.avg_cost, 0), nullif(mi.cost_price, 0), nullif(mi.buying_price, 0), 0)) as total_cost
      from public.orders o
      join eligible_orders eo on eo.id = o.id
      cross join lateral (
        select coalesce(nullif(btrim(coalesce(it->>'itemId', '')), ''), nullif(btrim(coalesce(it->>'menuItemId', '')), ''), nullif(btrim(coalesce(it->>'id', '')), '')) as item_id,
          case when lower(coalesce(it->>'unitType', '')) in ('gram','kg') and nullif(btrim(coalesce(it->>'weight', '')), '') is not null then coalesce(public.safe_cast_numeric(it->>'weight'), 0)
            else coalesce(public.safe_cast_numeric(it->>'quantity'), 0) * coalesce(public.safe_cast_numeric(it->>'uomQtyInBase'), public.safe_cast_numeric(it->>'uom_qty_in_base'), 1) end as qty
        from jsonb_array_elements(
          case when jsonb_typeof(o.data->'invoiceSnapshot'->'items') = 'array' and jsonb_array_length(o.data->'invoiceSnapshot'->'items') > 0 then o.data->'invoiceSnapshot'->'items'
            when jsonb_typeof(o.data->'items') = 'array' then o.data->'items' else '[]'::jsonb end
        ) as it
      ) q
      left join public.menu_items mi on mi.id::text = q.item_id::text
      left join public.stock_management sm on sm.item_id::text = q.item_id::text and sm.warehouse_id = o.warehouse_id
      where q.item_id is not null and q.qty > 0
      group by o.id
    )
    select coalesce(sum(coalesce(nullif(oic.total_cost, 0), nullif(im.total_cost, 0), coalesce(est.total_cost, 0))), 0)
    into v_total_cogs
  ...
  exception when others then v_total_cogs := 0;
  end;
  ...
  begin
    if to_regclass('public.inventory_movements') is not null and to_regclass('public.batches') is not null and to_regclass('public.sales_returns') is not null then
      ...
      ranged_orders as (
        select * from effective_orders eo
        where eo.date_by >= p_start_date and eo.date_by <= p_end_date and (p_zone_id is null or eo.zone_effective = p_zone_id)
      ),
      eligible_orders as (
        select o.id from public.orders o join ranged_orders ro on ro.id = o.id
        where o.status = 'delivered' or o.data->>'paidAt' is not null
      ...
      where im.reference_table = 'sales_returns' and im.movement_type = 'return_in'
        and im.batch_id is not null and sr.status = 'completed'
        and im.occurred_at >= p_start_date and im.occurred_at <= p_end_date
        and (p_warehouse_id is null or b.warehouse_id = p_warehouse_id);
    end if;
  exception when others then v_total_returns_cogs := 0;
  end;

  -- Wastage
  begin
    if to_regclass('public.wastage_records') is not null then
      select coalesce(sum(w.cost_amount), 0) into v_total_wastage
      from public.wastage_records w
      where w.status = 'approved' and w.created_at >= p_start_date and w.created_at <= p_end_date
        and (p_zone_id is null or w.zone_id = p_zone_id);
    end if;
  exception when others then v_total_wastage := 0;
  end;
  ...
  begin
    if to_regclass('public.expenses') is not null then
      ...
      where e.status = 'approved' and e.created_at >= p_start_date and e.created_at <= p_end_date
        and (p_zone_id is null or e.zone_id = p_zone_id);
    end if;
  exception when others then v_total_expenses := 0;
  end;

  -- Delivery costs
  begin
    if to_regclass('public.delivery_costs') is not null then
      select coalesce(sum(dc.cost_amount), 0) into v_total_delivery_cost
      from public.delivery_costs dc
      where dc.created_at >= p_start_date and dc.created_at <= p_end_date
        and (p_zone_id is null or dc.zone_id = p_zone_id);
    end if;
  exception when others then v_total_delivery_cost := 0;
  end;
  ...
  -- ═══════════════════════════════════════════════════════════════
  -- INVENTORY VALUE (current snapshot — standard practice)
  -- ═══════════════════════════════════════════════════════════════
  begin
    v_inventory_value := coalesce(public.get_inventory_valuation(p_warehouse_id), 0);
  exception when others then v_inventory_value := 0;
  end;

  -- ═══════════════════════════════════════════════════════════════
  -- PO STATUS COUNTS — ▶ NOW DATE-FILTERED
  -- ═══════════════════════════════════════════════════════════════
  select
  ...
  from (
    ...
    where (p_warehouse_id is null or po.warehouse_id = p_warehouse_id)
      and po.created_at >= p_start_date           -- ▶ NEW: date filter
      and po.created_at <= p_end_date              -- ▶ NEW: date filter
    group by po.status
  ) po;

  -- ═══════════════════════════════════════════════════════════════
  -- ORDER STATUS COUNTS — ▶ NOW DATE-FILTERED
  -- ═══════════════════════════════════════════════════════════════
  ...
  from (
    ...
    where o.status in ('pending','preparing','out_for_delivery','scheduled')
      and (p_warehouse_id is null or o.warehouse_id = p_warehouse_id)
      and nullif(trim(coalesce(o.data->>'voidedAt','')), '') is null
      and o.created_at >= p_start_date             -- ▶ NEW: date filter
      and o.created_at <= p_end_date               -- ▶ NEW: date filter
    group by o.status
  ) o;

  -- ═══════════════════════════════════════════════════════════════
  -- AR / AP — ▶ NOW DATE-FILTERED (transactions within period)
  -- ═══════════════════════════════════════════════════════════════
  begin
    if to_regclass('public.party_ledger_entries') is not null then
      -- AR: sum of debit entries within period for AR parties
      select coalesce(sum(
        case when ple.direction = 'debit' then coalesce(ple.amount, 0)
             when ple.direction = 'credit' then -coalesce(ple.amount, 0)
             else 0 end
      ), 0)
      into v_ar
      from public.party_ledger_entries ple
      join public.financial_parties fp on fp.id = ple.party_id
      where fp.party_type in ('customer','both')
        and ple.created_at >= p_start_date
        and ple.created_at <= p_end_date;

      -- AP: sum of credit entries within period for AP parties
      select coalesce(sum(
        case when ple.direction = 'credit' then coalesce(ple.amount, 0)
             when ple.direction = 'debit' then -coalesce(ple.amount, 0)
             else 0 end
      ), 0)
      into v_ap
      from public.party_ledger_entries ple
      join public.financial_parties fp on fp.id = ple.party_id
      where fp.party_type in ('supplier','both')
        and ple.created_at >= p_start_date
        and ple.created_at <= p_end_date;
    else
      -- Fallback to aging summaries (cumulative, not period-filtered)
      begin
        select coalesce(sum(s.total_outstanding), 0) into v_ar from public.party_ar_aging_summary s;
      exception when others then v_ar := 0;
      end;
      begin
        select coalesce(sum(s.total_outstanding), 0) into v_ap from public.party_ap_aging_summary s;
      exception when others then v_ap := 0;
      end;
    end if;
  ...
  -- ═══════════════════════════════════════════════════════════════
  -- PURCHASES — ▶ ALREADY DATE-FILTERED
  -- ═══════════════════════════════════════════════════════════════
  begin
    if to_regclass('public.inventory_movements') is not null and to_regclass('public.purchase_receipts') is not null then
      ...
      where im.reference_table = 'purchase_receipts' and im.movement_type = 'purchase_in'
        and im.occurred_at >= p_start_date and im.occurred_at <= p_end_date
        and (p_warehouse_id is null or pr.warehouse_id = p_warehouse_id);
    end if;
  exception when others then v_purchases_total := 0;
  end;
  ...
  begin
    if to_regclass('public.inventory_movements') is not null and to_regclass('public.purchase_returns') is not null and to_regclass('public.purchase_orders') is not null then
      ...
      where im.reference_table = 'purchase_returns' and im.movement_type = 'return_out'
        and im.occurred_at >= p_start_date and im.occurred_at <= p_end_date
        and (p_warehouse_id is null or po.warehouse_id = p_warehouse_id);
    end if;
  exception when others then v_purchase_returns_total := 0;
  end;
