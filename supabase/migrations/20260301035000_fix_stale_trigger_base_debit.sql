...
-- Fix trg_block_system_journal_lines_mutation which references 
-- non-existent columns base_debit and base_credit on journal_lines.
-- This causes errors when updating journal_lines (e.g. setting party_id).
...
begin
  ...
  if coalesce(v_source_table, '') <> '' and v_source_table <> 'manual' then
    ...
    -- (removed base_debit/base_credit which do not exist on journal_lines)
