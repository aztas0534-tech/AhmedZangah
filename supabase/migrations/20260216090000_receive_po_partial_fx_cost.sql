...
declare
  ...
  v_qty_base numeric;
  v_line_uom_id uuid;
  v_line_unit_cost_trx numeric;
  v_line_unit_cost_trx_base_uom numeric;
  v_line_unit_cost_base numeric;
begin
  ...
  v_po_currency := upper(coalesce(nullif(btrim(coalesce(v_po.currency, '')), ''), v_base));
  v_fx_date := coalesce(p_occurred_at, v_po.created_at, now())::date;
  v_fx := nullif(coalesce(v_po.fx_rate, 0), 0);

  if upper(v_po_currency) = upper(v_base) then
    v_fx := 1;
  elsif v_fx is null or v_fx <= 0 then
    v_fx := public.get_fx_rate(v_po_currency, v_fx_date, 'accounting');
    if v_fx is null or v_fx <= 0 then
      v_fx := public.get_fx_rate(v_po_currency, v_fx_date, 'operational');
    end if;
  end if;
  if v_fx is null or v_fx <= 0 then
    raise exception 'cannot resolve fx_rate for currency % on %', v_po_currency, v_fx_date;
  end if;
  ...
  loop
    ...
    for v_pi in
      select
        ...
        pi.unit_cost_foreign as unit_cost_foreign,
        pi.unit_cost_base as unit_cost_base
      from public.purchase_items pi
      where pi.purchase_order_id = p_order_id
        and pi.item_id = v_item_id
    ...
    loop
      ...
      v_line_uom_id := coalesce(
        v_pi.uom_id,
        v_uom_id,
        (select base_uom_id from public.item_uom where item_id = v_item_id limit 1)
      );

      v_line_unit_cost_trx := coalesce(v_pi.unit_cost_foreign, v_pi.unit_cost, 0);
      v_line_unit_cost_trx_base_uom := public.item_unit_cost_to_base(v_item_id, v_line_unit_cost_trx, v_line_uom_id);
      v_line_unit_cost_base := coalesce(
        v_pi.unit_cost_base,
        v_line_unit_cost_trx_base_uom * (case when upper(v_po_currency) = upper(v_base) then 1 else v_fx end)
      );
      if v_line_unit_cost_base is null or v_line_unit_cost_base < 0 then
        v_line_unit_cost_base := 0;
      end if;

      v_cost_sum := v_cost_sum + (v_take_qty * v_line_unit_cost_base);
      if upper(v_po_currency) <> upper(v_base) then
        v_cost_sum_foreign := v_cost_sum_foreign + (v_take_qty * greatest(coalesce(v_line_unit_cost_trx_base_uom, 0), 0));
      end if;
    ...
    into v_used_transport_cost, v_used_supply_tax_cost, v_category, v_is_food, v_expiry_required
    from public.menu_items mi
    where mi.id = v_item_id;

    if v_transport_cost is not null then
      v_used_transport_cost := v_transport_cost;
      if upper(v_po_currency) <> upper(v_base) then
        v_used_transport_cost := v_used_transport_cost * v_fx;
      end if;
    end if;
    if v_supply_tax_cost is not null then
      v_used_supply_tax_cost := v_supply_tax_cost;
      if upper(v_po_currency) <> upper(v_base) then
        v_used_supply_tax_cost := v_used_supply_tax_cost * v_fx;
      end if;
    end if;

    if v_old_qty < 0 then v_old_qty := 0; end if;
    if v_unit_cost < 0 then v_unit_cost := 0; end if;
    if v_used_transport_cost < 0 then v_used_transport_cost := 0; end if;
    if v_used_supply_tax_cost < 0 then v_used_supply_tax_cost := 0; end if;

    v_effective_unit_cost := v_unit_cost + coalesce(v_used_transport_cost, 0) + coalesce(v_used_supply_tax_cost, 0);
    ...
    values (
      ...
      case when upper(v_po_currency) <> upper(v_base) then v_po_currency else null end,
      case
        when upper(v_po_currency) <> upper(v_base) and v_fx > 0 then round(v_effective_unit_cost / v_fx, 6)
        else null
      end,
      case when upper(v_po_currency) <> upper(v_base) then v_fx else null end,
      case when upper(v_po_currency) <> upper(v_base) then v_fx_date else null end,
      v_qc_status,
      'active',
      jsonb_build_object(
