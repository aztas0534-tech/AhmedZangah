-- ============================================================================
-- Migration: Add dates to get_item_batches + batch date editing RPC
-- ============================================================================

-- Must DROP first because return type changed (added production_date, expiry_date)
...
begin
  ...
  left join lateral (
    ...
    where qc.batch_id = b.id and qc.check_type = 'inspection'
    order by qc.checked_at desc limit 1
  ) q on true
  where b.item_id = p_item_id::text
    and b.warehouse_id = v_wh
    and coalesce(b.status,'active') = 'active'
  group by b.id, b.created_at, b.unit_cost, b.foreign_unit_cost, b.foreign_currency,
           b.fx_rate_at_receipt, b.quantity_received, b.quantity_consumed,
           b.quantity_transferred, b.qc_status, q.last_result, q.last_at,
  ...
  begin
    select coalesce(qc_hold_quantity,0) into v_qc_hold
    from public.stock_management where item_id::text = p_item_id::text and warehouse_id = v_wh;
    ...
    if v_unbatched_qty > 0.001 then
      return query select
        ...
        now(), v_stock_avg, null::numeric, null::text, null::numeric,
        v_unbatched_qty, 0::numeric, v_unbatched_qty,
        'unbatched'::text, null::text, null::timestamptz,
        null::date, null::date;
    end if;
  end;
end;
...
declare
  v_batch record;
begin
  perform public._require_staff('update_batch_dates');
  if p_batch_id is null then
    raise exception 'batch_id is required';
  end if;

  select b.id, b.item_id, b.warehouse_id
  into v_batch from public.batches b where b.id = p_batch_id;
  if not found then raise exception 'batch not found'; end if;

  if p_production_date is not null and p_expiry_date is not null then
    if p_expiry_date <= p_production_date then
      raise exception 'تاريخ الانتهاء يجب أن يكون بعد تاريخ الإنتاج';
    end if;
  end if;
  ...
  if v_batch.warehouse_id is not null then
    perform public.recompute_stock_for_item(v_batch.item_id::text, v_batch.warehouse_id);
  end if;
end;
$$;

revoke all on function public.update_batch_dates(uuid, date, date) from public;
grant execute on function public.update_batch_dates(uuid, date, date) to authenticated;
