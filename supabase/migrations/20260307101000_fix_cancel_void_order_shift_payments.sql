...
-- When an order is cancelled or voided, any collected payments must be reversed not just in the ledger,
-- but also by dropping a 'direction=out' payment in the current cashier's open shift.
...
begin
  ...
  for v_mv in
    select p.id, p.method, p.amount, p.base_amount, p.currency, p.fx_rate
