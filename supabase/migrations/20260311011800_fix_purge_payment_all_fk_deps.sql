-- ═══════════════════════════════════════════════════════════════
-- purge_order_payment v9: Handle ALL FK dependencies comprehensively
-- Tables referencing payments: ar_allocations, ar_payment_status,
--   bank_reconciliation_matches
-- Tables referencing journal_entries: ar_open_items
-- Tables referencing journal_lines: party_open_items, party_ledger_entries
-- ═══════════════════════════════════════════════════════════════
...
begin
  ...
  -- ══════════════════════════════════════════════════════════════
  -- Phase 1: Delete settlement chain (settlement_lines → settlement_headers)
  -- ══════════════════════════════════════════════════════════════
  ...
  if array_length(v_payment_ids, 1) > 0 then
    -- ar_allocations → references payments(id)
    begin
      delete from public.ar_allocations where payment_id = any(v_payment_ids);
    exception when others then null;
    end;

    -- ar_payment_status → references payments(id)
    begin
      delete from public.ar_payment_status where payment_id = any(v_payment_ids);
    exception when others then null;
    end;

    -- bank_reconciliation_matches → references payments(id)
    begin
      delete from public.bank_reconciliation_matches where payment_id = any(v_payment_ids);
    exception when others then null;
    end;
  end if;
  ...
  -- ══════════════════════════════════════════════════════════════
  if array_length(v_jl_ids, 1) > 0 then
    -- party_open_items → references journal_lines(id)
    begin
    ...
    -- party_ledger_entries → references journal_lines(id)
    begin
    ...
    -- Delete journal_lines themselves
  ...
  if array_length(v_je_ids, 1) > 0 then
    -- ar_open_items → references journal_entries(id)
    begin
      delete from public.ar_open_items where journal_entry_id = any(v_je_ids);
    exception when others then null;
    end;
  ...
  -- ══════════════════════════════════════════════════════════════
  -- Phase 7: Update order and reopen AR (BEFORE re-enabling triggers)
  -- ══════════════════════════════════════════════════════════════

  -- Reset paidAt on order
