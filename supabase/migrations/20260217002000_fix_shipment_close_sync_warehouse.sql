-- Fix: Shipment Close Error & Restore Auto-Sync
-- 1. Restore the Auto-Sync logic (INSERT/UPDATE import_shipments_items from receipts) which was accidentally removed in the previous fix.
-- 2. Add `warehouse_id = new.destination_warehouse_id` filter to the Auto-Sync logic to ensure it matches the validation check.
-- 3. Retain the bypass logic and idempotent trigger fix.
...
begin
  ...
  with agg as (
    ...
    where pr.import_shipment_id = new.id
      and pr.warehouse_id = new.destination_warehouse_id -- FIXED: Filter by destination warehouse
  ...
  select
    ...
    v_base_currency, -- defaulted to base
    null,
  ...
  set
    ...
    -- preserve unit_price_fob if manually set? logic from previous migration implies keeping if > 0
