-- ═══════════════════════════════════════════════════════════════
-- 1) Restore party_ledger_entries for order #479F27 delivery entries
--    (they should NOT have been deleted - only payment entries)
-- 2) Revert purge function to only clean PAYMENT party_ledger_entries
-- ═══════════════════════════════════════════════════════════════

set app.allow_ledger_ddl = '1';

-- Step 1: Re-insert party_ledger_entries for order #479F27
do $$
declare
  v_order_id uuid;
  v_je_ids uuid[];
begin
  ...
  -- Get the order's journal entry IDs
  ...
  if array_length(v_je_ids, 1) > 0 then
    -- Re-run the party ledger insertion for each order journal entry
    for i in 1..array_length(v_je_ids, 1)
    loop
      begin
        perform public.insert_party_ledger_for_entry(v_je_ids[i]);
      exception when others then
        raise notice 'Could not re-insert party ledger for JE %: %', v_je_ids[i], sqlerrm;
      end;
    end loop;
  end if;

  -- Also restore AR open items if needed
  begin
    perform public.sync_ar_on_invoice(v_order_id);
  exception when others then
    raise notice 'Could not sync AR: %', sqlerrm;
  end;

  raise notice 'Restored party ledger entries for order %', v_order_id;
end $$;

-- Step 2: Fix purge function — only clean PAYMENT party_ledger_entries
...
begin
  ...
  -- ═══════════════════════════════════════════════════════
  -- Collect PAYMENT-related IDs only (NOT order entries)
  -- ═══════════════════════════════════════════════════════
  ...
  -- Delete settlement chain (payment-related only)
  ...
  -- Delete payment journal_line dependents
  if array_length(v_jl_ids, 1) > 0 then
  ...
  -- Delete payment journal entries
  if array_length(v_je_ids, 1) > 0 then
