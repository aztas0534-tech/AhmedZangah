-- Fix purge_order_payment: risk_level 'CRITICAL' → 'HIGH'
-- The check constraint only allows: LOW, MEDIUM, HIGH
...
begin
  ...
  -- Delete journal_line dependents
  if array_length(v_jl_ids, 1) > 0 then
  ...
  -- Delete journal_entry dependents then entries
  ...
  -- Audit log (risk_level must be LOW/MEDIUM/HIGH)
