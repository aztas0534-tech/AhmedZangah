...
begin
  ...
  values (
    ...
    auth.uid(),
    now(),
    jsonb_strip_nulls(jsonb_build_object('shiftId', p_shift_id::text, 'paymentId', v_payment_id::text, 'amount', v_amount, 'direction', v_dir, 'reason', nullif(trim(coalesce(p_reason, '')), ''), 'destinationAccountId', p_destination_account_id::text)),
    'MEDIUM',
    'SHIFT_CASH_MOVE'
  );
end;
...
-- Compatibility overload for older clients
create or replace function public.record_shift_cash_movement(
  p_shift_id uuid,
  p_direction text,
  p_amount numeric,
...
as $$
begin
  perform public.record_shift_cash_movement(p_shift_id, p_direction, p_amount, p_reason, p_occurred_at, null);
end;
$$;
...
-- Update post_payment to support cash_shifts cash movements
...
begin
  ...
  if coalesce(v_pay.reference_table, '') = 'cash_shifts' then
    ...
    if v_pay.direction = 'out' then
      insert into public.journal_lines(journal_entry_id, account_id, debit, credit, line_memo, currency_code, fx_rate, foreign_amount)
      values
        (v_entry_id, v_dest_account, v_amount_base, 0, 'Shift cash transfer out', v_cash_fx_code, v_cash_fx_rate, v_cash_fx_amount),
        (v_entry_id, v_credit_account, 0, v_amount_base, 'Cash out from shift', v_cash_fx_code, v_cash_fx_rate, v_cash_fx_amount);
    else
      insert into public.journal_lines(journal_entry_id, account_id, debit, credit, line_memo, currency_code, fx_rate, foreign_amount)
      values
        (v_entry_id, v_debit_account, v_amount_base, 0, 'Cash in to shift', v_cash_fx_code, v_cash_fx_rate, v_cash_fx_amount),
        (v_entry_id, v_dest_account, 0, v_amount_base, 'Shift cash transfer in', v_cash_fx_code, v_cash_fx_rate, v_cash_fx_amount);
    end if;
    perform public.check_journal_entry_balance(v_entry_id);
    return;
  end if;
