...
begin
  ...
  where b.item_id::text = new.item_id::text
    and (b.warehouse_id = new.warehouse_id or b.warehouse_id is null)
  ...
  if v_batch_id is null then
    ...
    values (
      ...
      new.item_id::text,
      new.warehouse_id,
      concat('TRF-AUTO-', substring(coalesce(new.reference_id, new.id::text) from 1 for 8)),
      greatest(coalesce(new.quantity, 0), 0),
      0,
      coalesce(new.unit_cost, 0),
      'released',
      jsonb_build_object('autoCreatedForTransfer', true, 'referenceTable', new.reference_table, 'referenceId', new.reference_id, 'movementType', new.movement_type)
    );
  end if;
