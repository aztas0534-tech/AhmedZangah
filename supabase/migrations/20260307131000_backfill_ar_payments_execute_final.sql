-- Create a SECURITY DEFINER function to backfill AR payments, spoofing auth.uid() to bypass post_payment permission checks
set app.allow_ledger_ddl = '1';

create or replace function public.backfill_ar_payments_final()
returns jsonb
language plpgsql
security definer
...
declare
  ...
  v_admin_id uuid;
begin
  -- Spoof auth.uid() to an active owner so that post_payment triggers don't throw "not allowed"
  select auth_user_id into v_admin_id from public.admin_users where role = 'owner' and is_active = true limit 1;
  ...
  loop
    ...
    begin
      ...
      if upper(coalesce(v_order.currency, 'YER')) = upper(coalesce(v_order.base_currency, 'SAR')) then 
      ...
      -- Use the spoofed admin ID if order creator is not known
      insert into public.payments (
        ...
        v_order.id::text,
        v_order.created_at,
        coalesce(v_admin_id, (select auth_user_id from admin_users limit 1)),
...
SELECT public.backfill_ar_payments_final();
