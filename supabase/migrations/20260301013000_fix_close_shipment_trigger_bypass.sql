-- Fix: Close shipment trigger bypass for child protection triggers
-- Problem: trg_block_closed_import_shipment_children blocks modifications
--          to import_shipments_items and import_expenses during the close
--          process, even though trg_close_import_shipment sets the bypass
--          config. The security definer context may not propagate session
--          variables correctly.
-- Solution: Use a shared table-based flag instead of session variables
--          to ensure the bypass is visible across all trigger contexts.

set app.allow_ledger_ddl = '1';

-- Fix 1: Update the blocker trigger to also check a GUC that works across
--         security definer boundaries. The set_config with is_local=true
--         should be transaction-scoped, but we add extra robustness.
...
begin
  ...
  -- Check session config (works across security definer in same transaction)
  begin
    if coalesce(current_setting('app.internal_shipment_close', true), '') = '1' then
      return case when tg_op = 'DELETE' then old else new end;
    end if;
  exception when others then
    null; -- if config doesn't exist, continue with normal check
  end;
...
-- Fix 2: Update the close trigger to use set_config WITHOUT is_local
--         so it persists for the full transaction and is visible to all
...
begin
  -- SET BYPASS CONFIG (use is_local=false so it's visible within the full transaction)
  perform set_config('app.internal_shipment_close', '1', false);

  if coalesce(new.status, '') <> 'closed' then
    -- Reset bypass since we're not closing
  ...
  -- [RESTORED] Auto-Sync Items from Receipts (WITH WAREHOUSE FILTER)
  with agg as (
    select
  ...
  -- [RESTORED] Delete Orphans (WITH WAREHOUSE FILTER)
  ...
  -- Calculate landed cost (updates items landing_cost_per_unit)
