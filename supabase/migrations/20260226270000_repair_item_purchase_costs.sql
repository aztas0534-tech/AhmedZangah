...
begin
  ...
  if to_regclass('public.purchase_orders') is null
    ...
    raise exception 'required tables missing';
  end if;
  ...
  if coalesce(p_dry_run, true) then
    with pi_avg as (
              ...
              * coalesce(
                  ...
                  case
                    when upper(coalesce(po.currency, v_base)) = v_base then
                      ...
                      public.item_unit_cost_to_base(pi.item_id, coalesce(pi.unit_cost_foreign, pi.unit_cost, 0), pi.uom_id) * coalesce(nullif(po.fx_rate, 0), 1)
                  end,
                  0
                )
      ...
      group by pr.id, b.item_id::text, upper(coalesce(po.currency, v_base)), coalesce(nullif(po.fx_rate, 0), 1)
    ),
    pri_fix as (
      select
        pri.id as pri_id,
