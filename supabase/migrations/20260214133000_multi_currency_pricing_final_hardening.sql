...
create or replace function public.lock_batch_foreign_snapshot()
returns trigger
language plpgsql
security definer
...
begin
  if to_regclass('public.batches') is not null then
    ...
    drop trigger if exists trg_lock_batch_foreign_snapshot on public.batches;
    create trigger trg_lock_batch_foreign_snapshot
    before update on public.batches
    for each row
    when (old.foreign_unit_cost is not null)
    execute function public.lock_batch_foreign_snapshot();
  end if;
end $$;
...
begin
  ...
  if to_regclass('public.app_settings') is not null then
    ...
    limit 1;
    begin
      v_env := nullif(btrim(coalesce(v_settings->'settings'->>'environment','')), '');
    exception when others then
      v_env := null;
    end;
  end if;
  return coalesce(lower(v_env) = 'production', false);
end;
$$;
