-- ═══════════════════════════════════════════════════════════════
-- Fix #4: Decrease purchase_items.received_quantity after return
-- Fix #9: Call reconcile_purchase_order_receipt_status after return
--
-- Applied on top of 20260311013300 which already fixed:
--   #1 UOM conversion, #2 avg_cost recalc, #3 base_unit
-- ═══════════════════════════════════════════════════════════════
...
declare
  ...
  v_old_avg numeric;
  v_new_qty numeric;
  v_deducted_cost numeric;
  v_new_avg numeric;
  v_pi_remaining numeric;              -- FIX #4: for distributing returned qty across purchase_items
  v_pi_take numeric;                   -- FIX #4
begin
  ...
  loop
    ...
    -- ══════════════════════════════════════════════════════════
    -- FIX #4: Decrease purchase_items.received_quantity
    -- Distribute the returned qty across purchase_items rows (FIFO)
    -- so that PO status can correctly revert to 'partial'
    -- ══════════════════════════════════════════════════════════
    v_pi_remaining := v_qty_base;
    for v_batch in
      select pi.id, coalesce(pi.received_quantity, 0) as recv_qty
      from public.purchase_items pi
      where pi.purchase_order_id = p_order_id
        and pi.item_id::text = v_item_id_text
        and coalesce(pi.received_quantity, 0) > 0
      order by pi.created_at asc, pi.id asc
      for update
    loop
      exit when v_pi_remaining <= 0;
      v_pi_take := least(v_pi_remaining, v_batch.recv_qty);
      if v_pi_take <= 0 then continue; end if;

      update public.purchase_items
      set received_quantity = greatest(0, coalesce(received_quantity, 0) - v_pi_take)
      where id = v_batch.id;

      v_pi_remaining := v_pi_remaining - v_pi_take;
    end loop;
    ...
    -- ══════════════════════════════════════════════════════════
    v_old_qty := coalesce(v_stock_available, 0);
  ...
  -- ══════════════════════════════════════════════════════════
  -- FIX #9: Reconcile purchase order status after return
  -- This will set PO status back to 'partial' if received_quantity
  -- is now less than ordered quantity for any item
  -- ══════════════════════════════════════════════════════════
  begin
  ...
  exception when others then
    null; -- non-critical, don't fail the return
  end;
