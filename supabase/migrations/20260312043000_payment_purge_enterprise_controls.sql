...
create table if not exists public.order_payment_purge_requests (
  id uuid primary key default gen_random_uuid(),
  order_id uuid not null references public.orders(id) on delete cascade,
  status text not null default 'requested' check (status in ('requested','executed','rejected','cancelled')),
  reason text not null,
  reason_category text not null check (reason_category in ('misapplied_payment','duplicate_settlement','fraud_risk','compliance_correction','other')),
  ...
  locked_period_violation boolean not null default false,
  metadata jsonb not null default '{}'::jsonb,
  check (char_length(trim(reason)) >= 20)
);

create index if not exists idx_order_payment_purge_requests_order_status
  on public.order_payment_purge_requests(order_id, status, requested_at desc);
create unique index if not exists uq_order_payment_purge_requests_open
  on public.order_payment_purge_requests(order_id)
  where status = 'requested';

alter table public.order_payment_purge_requests enable row level security;

drop policy if exists order_payment_purge_requests_select on public.order_payment_purge_requests;
create policy order_payment_purge_requests_select
on public.order_payment_purge_requests
for select
using (
  public.has_admin_permission('orders.view')
  or public.has_admin_permission('accounting.view')
  or public.has_admin_permission('accounting.manage')
  or public.is_admin()
);

drop policy if exists order_payment_purge_requests_insert on public.order_payment_purge_requests;
create policy order_payment_purge_requests_insert
on public.order_payment_purge_requests
for insert
with check (
  public.is_admin()
...
drop policy if exists order_payment_purge_requests_update on public.order_payment_purge_requests;
create policy order_payment_purge_requests_update
on public.order_payment_purge_requests
for update
using (
  public.is_admin()
...
create or replace function public.request_order_payment_purge(
  p_order_id uuid,
  p_reason text,
...
declare
  ...
  v_category text := lower(trim(coalesce(p_reason_category, '')));
...
begin
  ...
  if v_category not in ('misapplied_payment','duplicate_settlement','fraud_risk','compliance_correction','other') then
  ...
  if not v_has_payments then
    raise exception 'no posted payments found for this order';
  end if;
  ...
  if exists (
    ...
    raise exception 'an open purge request already exists';
  end if;

  insert into public.order_payment_purge_requests(
    order_id, status, reason, reason_category, requested_by, metadata
  ) values (
    p_order_id, 'requested', v_reason, v_category, auth.uid(),
    jsonb_build_object(
      'requestedFrom', 'manage_orders',
      'reasonCategory', v_category
    )
  )
  ...
  set data = coalesce(data, '{}'::jsonb) || jsonb_build_object(
    'paymentPurgeRequestId', v_request_id::text,
    'paymentPurgeRequestedAt', now()::text,
    'paymentPurgeRequestedBy', auth.uid()::text,
    'paymentPurgeReason', v_reason,
    'paymentPurgeReasonCategory', v_category
  ),
  updated_at = now()
  where id = p_order_id;
  ...
  insert into public.system_audit_logs(
    ...
    'payment.purge.request',
    'payments',
    concat('Requested payment purge for order ', right(p_order_id::text, 8)),
    auth.uid(),
    now(),
    jsonb_build_object(
...
as $$
begin
  raise exception 'hard-delete purge disabled; use request_order_payment_purge + approve_order_payment_purge';
end;
$$;

revoke all on function public.request_order_payment_purge(uuid, text, text) from public;
grant execute on function public.request_order_payment_purge(uuid, text, text) to authenticated;
revoke all on function public.approve_order_payment_purge(uuid, text) from public;
grant execute on function public.approve_order_payment_purge(uuid, text) to authenticated;
