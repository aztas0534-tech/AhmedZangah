  ...
  begin
    ...
    loop
      select
        ...
        max(case when coalesce(jl.debit, 0) > 0 then jl.id else null end),
        max(case when coalesce(jl.credit, 0) > 0 then jl.id else null end),
        max(coalesce(jl.debit, 0)) filter (where coalesce(jl.debit, 0) > 0),
        max(coalesce(jl.credit, 0)) filter (where coalesce(jl.credit, 0) > 0)
      into
        v_line_count,
        v_debit_lines,
        v_credit_lines,
      ...
      if v_line_count <> 2 or v_debit_lines <> 1 or v_credit_lines <> 1 then
