-- ═══════════════════════════════════════════════════════════════
-- purge_order_payment v12: Also clean party_ledger_entries and
-- party_open_items for the ORDER's own journal entries
-- (delivery/invoicing), not just payment entries
-- ═══════════════════════════════════════════════════════════════
...
declare
  ...
  v_order_je_ids uuid[];    -- order delivery/invoicing journal entries
  v_all_je_ids uuid[];      -- combined
  v_jl_ids uuid[];
  v_doc_ids uuid[];
...
begin
  ...
  -- Journal entries from the ORDER itself (delivery/invoicing/etc)
  ...
  v_all_je_ids := v_je_ids || v_order_je_ids;

  -- ALL journal line IDs (from both payment and order entries)
  select coalesce(array_agg(jl.id), '{}') into v_jl_ids
  from public.journal_lines jl
  where jl.journal_entry_id = any(v_all_je_ids);

  -- Document IDs (only from payment entries, order docs stay)
  select coalesce(array_agg(distinct je.document_id), '{}') into v_doc_ids
  from public.journal_entries je
  where je.id = any(v_je_ids) and je.document_id is not null;

  -- ══════════════════════════════════════════════════════════════
  -- Delete settlement chain
  -- ══════════════════════════════════════════════════════════════
  ...
  -- ══════════════════════════════════════════════════════════════
  -- Delete journal_line dependents (BOTH payment + order entries)
  -- ══════════════════════════════════════════════════════════════
  if array_length(v_jl_ids, 1) > 0 then
  ...
  -- ══════════════════════════════════════════════════════════════
  -- Delete journal lines for PAYMENT entries (order entries keep lines)
  -- ══════════════════════════════════════════════════════════════
  ...
  -- ══════════════════════════════════════════════════════════════
  -- Delete journal_entry dependents + PAYMENT entries
  -- ══════════════════════════════════════════════════════════════
  if array_length(v_je_ids, 1) > 0 then
  ...
  -- Also delete AR open items for order entry IDs
  if array_length(v_order_je_ids, 1) > 0 then
    begin delete from public.ar_open_items where journal_entry_id = any(v_order_je_ids); exception when others then null; end;
  end if;
  ...
  -- ══════════════════════════════════════════════════════════════
  -- Delete accounting_documents (payment-related only)
  -- ══════════════════════════════════════════════════════════════
  ...
  -- ══════════════════════════════════════════════════════════════
  -- Update order (while triggers still disabled)
  -- ══════════════════════════════════════════════════════════════
