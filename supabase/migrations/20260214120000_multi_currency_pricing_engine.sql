...
begin
  ...
  if not exists (select 1 from pg_type where typname = 'pricing_fx_source_enum') then
    create type public.pricing_fx_source_enum as enum (
      'NONE',
      'OPERATIONAL',
      'ACCOUNTING',
      'PURCHASE_SNAPSHOT'
...
create table if not exists public.product_prices_multi_currency (
  id uuid primary key default gen_random_uuid(),
  item_id text not null references public.menu_items(id) on delete cascade,
  currency_code text not null references public.currencies(code),
  pricing_method public.pricing_method_enum not null,
  price_value numeric,
  margin_percent numeric,
  fx_source public.pricing_fx_source_enum not null default 'NONE',
  is_active boolean not null default true,
  effective_from date,
  effective_to date,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint ppmc_price_value_check check (price_value is null or price_value >= 0),
  constraint ppmc_margin_percent_check check (margin_percent is null or margin_percent >= 0)
);

create index if not exists idx_ppmc_item_currency_active on public.product_prices_multi_currency(item_id, currency_code, is_active);
create index if not exists idx_ppmc_effective on public.product_prices_multi_currency(effective_from, effective_to);

create or replace function public.is_multi_currency_pricing_enabled()
returns boolean
language plpgsql
stable
security definer
...
begin
  ...
  if to_regclass('public.app_settings') is not null then
    ...
    limit 1;
    begin
      v_flag := coalesce((v_settings->'settings'->>'ENABLE_MULTI_CURRENCY_PRICING')::boolean, false);
    ...
    if found then
      ...
      if v_row.pricing_method in ('FIXED','MANUAL_OVERRIDE') then
        ...
        if v_fx_source = 'OPERATIONAL' then
          ...
          where upper(coalesce(po.currency, '')) = v_currency
        ...
        select nullif(pi.unit_cost_foreign, 0)
        into v_foreign_cost
...
begin
  ...
  where not exists (
    select 1
    from public.product_prices_multi_currency ppmc
    where ppmc.item_id = mi.id
      and upper(ppmc.currency_code) = upper(v_base)
      and ppmc.is_active = true
  );

  if upper(v_base) <> 'YER' then
    ...
    if v_fx is not null and v_fx > 0 then
      ...
      select
        ...
        'YER',
        'FIXED',
        coalesce(nullif((mi.data->>'price')::numeric, null), mi.price, 0) / v_fx,
        'OPERATIONAL',
        true,
        current_date
      from public.menu_items mi
      where not exists (
        ...
        from public.product_prices_multi_currency ppmc
        where ppmc.item_id = mi.id
          and upper(ppmc.currency_code) = 'YER'
          and ppmc.is_active = true
      );
    end if;
  end if;
...
begin
  ...
  v_base_price := public.resolve_item_price(p_item_id::text, public.get_base_currency(), v_qty, current_date, p_customer_id);
