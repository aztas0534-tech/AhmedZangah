-- =============================================================================
-- ميزة الصرف والإيداع للأطراف المالية والحسابات المختلفة
-- =============================================================================
...
create or replace function public.record_shift_cash_movement(
  ...
  p_destination_party_id uuid default null
)
returns void
language plpgsql
security definer
...
begin
  ...
  if p_destination_party_id is not null then
    if not exists (select 1 from public.financial_parties where id = p_destination_party_id and status = 'active') then
      raise exception 'invalid destination party';
    end if;
    if p_destination_account_id is not null then
      raise exception 'cannot provide both account and party';
    end if;
  end if;
  ...
  if p_destination_party_id is not null then
    v_payment_data := v_payment_data || jsonb_build_object('destinationPartyId', p_destination_party_id::text);
  end if;
  ...
  values (
    ...
    jsonb_strip_nulls(jsonb_build_object(
      ...
      'destinationPartyId', p_destination_party_id::text
    )),
    'MEDIUM',
    'SHIFT_CASH_MOVE'
  );
...
revoke all on function public.record_shift_cash_movement(uuid, text, numeric, text, timestamptz, uuid, text, numeric, uuid) from public;
grant execute on function public.record_shift_cash_movement(uuid, text, numeric, text, timestamptz, uuid, text, numeric, uuid) to anon, authenticated;
...
as $$
begin
  perform public.record_shift_cash_movement(p_shift_id, p_direction, p_amount, p_reason, p_occurred_at, p_destination_account_id, p_currency, p_fx_rate, null);
end;
$$;
...
--------------------------------------------------------------------------------
-- Update post_payment to handle destinationPartyId
--------------------------------------------------------------------------------
  ...
  if coalesce(v_pay.reference_table, '') = 'cash_shifts' then
    ...
    if v_party_id is not null then
      ...
      if v_party is not null then
        ...
        v_dest_account := case when v_method = 'cash' then v_cash else v_bank end;
      end if;
    else
