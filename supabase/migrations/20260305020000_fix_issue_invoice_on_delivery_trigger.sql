-- Fix issue_invoice_on_delivery trigger function which was referencing non-existent column
...
begin
  if new.status = 'delivered' and (old.status is distinct from new.status) then
    ...
    if not coalesce(v_has_invoice, false) then
      ...
      -- REMOVED: new.invoice_issued_at := v_issued_at; -- Does not exist on public.orders
