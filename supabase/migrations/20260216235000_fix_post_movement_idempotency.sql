-- Fix: Make post_inventory_movement idempotent
-- The previous version raised an exception if the entry existed, which causes issues if the trigger fires multiple times (e.g. on update).
-- This version simply returns if the entry already exists.
...
begin
  ...
  if exists (
    ...
    -- FIXED: Idempotency - Just return if already posted
    return;
  end if;
