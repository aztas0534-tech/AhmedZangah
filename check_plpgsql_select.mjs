...
const funcsStr = fs.readFileSync('formatted_funcs.txt', 'utf8');
const lines = funcsStr.split('\n');

let results = [];
for (let i = 0; i < lines.length; i++) {
  const line = lines[i].toLowerCase();
  
  if (line.includes('select ') && (line.includes(' data') || line.includes('*.data'))) {
    for (const t of missingTables) {
      if (line.includes(t)) {
        // Exclude lines that ALSO explicitly name a valid table, just to reduce noise, unless the valid table is completely irrelevant.
        let skip = false;
        if (line.includes('orders') || line.includes('batches') || line.includes('inventory_movements') || line.includes('stock_management')) {
          skip = true;
        }
        
        if (!skip) {
           results.push({ lineNum: i + 1, table: t, text: lines[i].trim() });
        }
      }
    }
  }
...
if (results.length > 0) {
    console.log("Found suspicious SELECT data on missing data columns!!");
    console.log(JSON.stringify(results, null, 2));
} else {
    console.log("No SELECT data conflicts found across " + missingTables.length + " tables in all functions.");
}
