      ...
      <div className="document-container w-full mx-auto p-12 bg-[#FAFAFA] flex flex-col text-blue-950 print:p-0" style={{ fontFamily: 'Tajawal, Cairo, sans-serif' }}>
        ...
        <div className="luxury-header relative z-10 flex flex-col md:flex-row justify-between items-center md:items-end gap-6 pb-6 mb-8 border-b-2 border-slate-900 print:pb-0 print:mb-0 print:border-none print:flex-row">
          ...
          <div className={`text-center flex flex-col items-center flex-shrink-0 z-10 ${isArabic ? 'md:text-left rtl:text-left' : 'md:text-right rtl:text-right'}`}>
            <h2 className="doc-title">{isArabic ? 'نقل مستودعي' : 'Warehouse Transfer'}</h2>
            <div className="title-sub">TRANSFER DOCUMENT</div>
            {data.documentStatus && (
        ...
        <div className="info-grid relative z-10 mb-8 print:mb-4">
          <div className="info-group">
            <div className="info-item mb-2 print:mb-1">
              <span className="font-thin-label">{isArabic ? 'من المستودع | From WH' : 'From Warehouse'}</span>
              <span className="font-bold-value text-gold">{data.fromWarehouseName || '—'}</span>
            </div>
            <div className="info-item">
              <span className="font-thin-label">{isArabic ? 'إلى المستودع | To WH' : 'To Warehouse'}</span>
              <span className="font-bold-value text-gold">{data.toWarehouseName || '—'}</span>
            </div>
          </div>

          <div className="info-group border-x border-slate-300 px-4 print:border-[#E5E7EB]">
            <div className="info-item mb-2 print:mb-1">
              <span className="font-thin-label">{isArabic ? 'رقم النقل | TR No.' : 'TR No.'}</span>
              <span className="font-bold-value font-mono text-charcoal" dir="ltr">#{data.transferNumber}</span>
            </div>
            <div className="info-item">
              <span className="font-thin-label">{isArabic ? 'التاريخ | Date' : 'Date'}</span>
              <span className="font-bold-value font-mono tabular" dir="ltr">{new Date(data.transferDate).toLocaleDateString('en-GB')}</span>
            </div>
          </div>
          ...
          <div className="info-group border-r border-slate-300 pr-4 print:border-[#E5E7EB]">
            <div className="info-item mb-2 print:mb-1">
              ...
              <span className="font-bold-value font-mono text-charcoal tabular" dir="ltr">{data.referenceId || '—'}</span>
            </div>
            <div className="info-item">
              <span className="font-thin-label">{isArabic ? 'الحالة | Status' : 'Status'}</span>
          ...
          <div className="flex justify-around items-end px-12 print:px-8">
            <div className="text-center w-32 print:w-28">
              <div className="border-t border-blue-900 print:border-blue-900 pt-1.5">
                <span className="font-thin-label block text-blue-950 font-bold">{isArabic ? 'المُرسل (أمين المخزن) | Sender' : 'Sender'}</span>
              </div>
            </div>
            <div className="text-center w-32 print:w-28">
              <div className="border-t border-blue-900 print:border-blue-900 pt-1.5">
                <span className="font-thin-label block text-blue-950 font-bold">{isArabic ? 'الناقل | Transporter' : 'Transporter'}</span>
              </div>
            </div>
            <div className="text-center w-32 print:w-28">
              <div className="border-t border-blue-900 print:border-blue-900 pt-1.5">
                <span className="font-thin-label block text-blue-950 font-bold">{isArabic ? 'المُستلم | Receiver' : 'Receiver'}</span>
              </div>
            </div>
          </div>
        </div>
