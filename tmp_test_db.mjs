...
async function run() {
    ...
    console.log("PO Res:", res.error || res.data);

    if (res.data?.[0]?.id) {
        const poId = res.data[0].id;
        console.log("Inserting Item...");
        const itemRes = await supabase.from('purchase_items').insert([{
            purchase_order_id: poId,
            item_id: 'any_item_id_here', // intentionally invalid to see if it hits foreign key or not null first
            quantity: 1,
            unit_cost: 100,
            unit_cost_foreign: 100,
            unit_cost_base: 100,
            total_cost: 100
        }]);
        console.log("Item Res:", itemRes.error || itemRes.data);
